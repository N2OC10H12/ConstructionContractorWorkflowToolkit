...
public class EstimatePdfPrintableRowPartitionService {
    ...
    public EstimatePdfPrintableRowPartition partition(
        ...
        List<EstimatePdfPrintableRow> selectedCarryDetails =
                detailRows.subList(
                        carryStartIndex,
                        detailRows.size());
        ...
        Set<EstimatePdfPrintableRow> selectedCarryDetailSet =
                newIdentitySet();
        ...
        ContextRowIndex contextRowIndex =
                new ContextRowIndex(printableRows);
    ...
    private List<EstimatePdfPrintableRow> buildFinalCarryRows(
        ...
        EstimatePdfItemTypeGroup currentItemType = null;
        ...
        for (EstimatePdfPrintableRow detail : selectedDetails) {
            ...
            if ((displayMode == CustomerDisplayMode.ITEM_LEVEL
                    ...
                    && detail.getItemType() != currentItemType) {

                addCarryContext(
                        carryRows,
                        contextRowIndex.itemTypeRow(
                                detail.getItemType()),
                ...
                currentItemType = detail.getItemType();
                currentItem = null;
            }

            if (displayMode == CustomerDisplayMode.ITEM_COST_LEVEL
                    && detail.getRowType()
                    == EstimatePdfPrintableRowType.COST
            ...
            if (detail.getRowType()
                    == EstimatePdfPrintableRowType.ITEM) {
                currentItem = detail.getItem();
            }
        }
    ...
    private boolean isDetailRow(
        ...
        return switch (displayMode) {
            case GROUP_LEVEL ->
                    row.getRowType()
                            == EstimatePdfPrintableRowType.GROUP;

            case ITEM_TYPE_LEVEL ->
                    row.getRowType()
                            == EstimatePdfPrintableRowType.ITEM_TYPE;
    ...
    private static class ContextRowIndex {
        ...
        private final Map<EstimatePdfGroup, EstimatePdfPrintableRow>
                groupRows = new IdentityHashMap<>();

        private final Map<EstimatePdfItemTypeGroup, EstimatePdfPrintableRow>
                itemTypeRows = new IdentityHashMap<>();

        private final Map<EstimatePdfItemLine, EstimatePdfPrintableRow>
                itemRows = new IdentityHashMap<>();
        ...
        private ContextRowIndex(
            ...
            for (EstimatePdfPrintableRow row : printableRows) {
                ...
                } else if (row.getRowType()
                        == EstimatePdfPrintableRowType.ITEM_TYPE) {
                    itemTypeRows.put(row.getItemType(), row);
                } else if (row.getRowType()
                        == EstimatePdfPrintableRowType.ITEM) {
        ...
        private EstimatePdfPrintableRow itemTypeRow(
                EstimatePdfItemTypeGroup itemType
        ) {
            return itemTypeRows.get(itemType);
        }
